...
<?php
	...
	//randomly generated quizid
	$idchck=1;
	while($idchck>0)
	{
	$quizid=random(1,9999);
	$idquery="Select count(*) from Quiz where quiz ID=$id";
	$idchck=mysql_query($idquery);
	}
	...
	$q="INSERT INTO quiz(Quiz Id,Subject Id, Board ID, Question ID,Type,class) values ($quiz_id,$subj_id,$board_id,$type, $class)";
	mysql_query($q) or die("Error executing query:".mysql_error());
	
	$query="INSERT INTO quiz_questions(Quiz Id,Subject Id, Board ID, Question ID, ques, opt1, opt2, opt3, opt4, correct_ans) VALUES ($quiz_id,$subject_id,$board_id,$quesid,$A,$B,$C,$D,$correct_ans)";
