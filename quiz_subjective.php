...
<?php
	if($_SESSION['previlege']==0)
			...
			$sql3=mysql_query("SELECT * FROM quiz WHERE Quiz_ID ='$quizid' and Type='Sub'");
			$quiz=mysql_fetch_assoc($sql3);
			
			$sql2=mysql_query("SELECT Subject_ID FROM package_having_subject NATURAL JOIN student NATURAL JOIN subject WHERE Student_ID ='$id' and Subject_Name='$subject'");	
			$sub_id=mysql_fetch_assoc($sql2);
		    $subid=$sub_id['Subject_ID'];
			
			//$fetch=mysql_fetch_assoc($result);
 $sql4=mysql_query( "SELECT * FROM faculty NATURAL JOIN quiz where Subject_ID='$subid' and Board_ID='$board1' and Quiz_ID='$quizid'");
		//	$result4=mysql_query($sql4);
			$fetch4=mysql_fetch_assoc($sql4);
		
			$fac_name=$fetch4['Faculty_Name'];
			
			$time=$fetch4['Timestamp'];
			//$board=mysql_fetch_assoc($sql2);
			
		  
          echo "<h3>".$quiz['Quiz_name'].",".$subject." </h3> <hr>
          <h4>added by ".$fac_name." on ".$time." </h4>";
		  ?>
    
	<h5>Instructions: ----------------</h5>
          <hr>
          <form autocomplete="off" action="<?php echo "quiz_sub_sol.php?link=".$subid."&link1=".$quizid; ?>" method="post" name="form"  />
            <fieldset>
              <ol>
			  <?php
			  $sql6=mysql_query("select * from quiz_questions where Subject_ID='$subid' and Board_ID='$board1' and Quiz_ID='$quizid' order by Question_ID");
			while( $fetch6=mysql_fetch_assoc($sql6))
			{
			
			  
                echo "<li> : ".$fetch6['Statement'].
            ...
            <input type="submit" value="View Solutions" class="btn btn-large btn-inverse span3" data-toggle="modal"  style="margin-left:40%" >
          </fieldset>
        </form>
